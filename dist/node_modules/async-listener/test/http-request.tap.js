...
test('http.Agent socket reuse works', function (t) {
  function main(done) {
    ...
    function after(rand, i) {
      if (--times === 0) {
        t.deepEqual(listener.root, expected, 'should have equal state structures');
        if (agent.destroy) {
          agent.destroy();
        }
        done();
    ...
    function ping(i) {
      ...
      var req = http.request({
        ...
        if (i === 0) {
          t.equal(listener.current.name, 'ping #' + i + ' request', 'should be ping #' + i + ' request');
        } else {
          t.equal(listener.current.name, 'setImmediate to after #' + (i - 1), 'should be setImmediate to after #' + (i - 1));
        }
        ...
        res.on('end', function () {
          ...
          t.equal(listener.current.name, 'res.resume ping #' + i, 'should be res.resume ping #' + i);
    ...
    var innerResumeChildrenWrapped = [make('res.resume ping #0', {
      children: innerResumeChildren
    }), make('res.resume ping #0'), make('res.resume ping #0')];
    var innerPingChildren = [];
    if (nodeVersion[0] == 0 && nodeVersion[1] < 12) {
      innerPingChildren.push(make('res.resume ping #0'));
    }
    innerPingChildren.push(make('res.resume ping #0', {
      children: nodeVersion[0] == 0 && nodeVersion[1] < 12 ? innerResumeChildren : innerResumeChildrenWrapped
    }));
    ...
    var firstImmediateChildren = [make('ping #0 request', {
      children: [make('ping #0 request', {
        children: innerPingChildren
      }), make('ping #0 request', {
        children: nodeVersion[0] > 0 || nodeVersion[1] > 10 ? [make('req.end ping #1')] : []
      })]
    })];
    ...
    firstImmediateChildren.push(make('ping #0 request'), make('ping #0 request', {
      before: 0,
      after: 0
    }));
    ...
    var innerPingChildrenWrapped = [make('res.resume ping #1', {
      children: innerPingChildren
    }), make('res.resume ping #1'), make('res.resume ping #1')];
    var innerImmediateChildren = [];
    if (nodeVersion[0] == 0 && nodeVersion[1] < 12) {
      innerImmediateChildren.push(make('res.resume ping #1'));
    }
    innerImmediateChildren.push(make('res.resume ping #1', {
      children: nodeVersion[0] == 0 && nodeVersion[1] < 12 ? innerPingChildren : innerPingChildrenWrapped
    }));
    if (nodeVersion[0] > 0 || nodeVersion[1] > 10) {
      if (nodeVersion[0] < 6 && nodeVersion[0] !== 4) {
        innerImmediateChildren.push(make('res.resume ping #1'));
    ...
    var expected = make('root', {
      children: [firstImmediate, secondImmediate],
      before: 0,
      after: 0
    });
  }
...
//# sourceMappingURL=http-request.tap.js.map