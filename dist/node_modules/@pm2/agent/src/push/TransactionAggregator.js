...
var log = require('debug')('interactor:aggregator');
var Utility = require('../Utility.js');
var fclone = require('fclone');
var Histogram = require('../utils/probes/Histogram');
...
module.exports = function () {
  function TransactionAggregator(pushInteractor) {
    _classCallCheck(this, TransactionAggregator);
  ...
  _createClass(TransactionAggregator, [{
    key: 'init',
    value: function init() {
      var _this = this;

      // New Process Online, reset & wait a bit before processing
      this.pushInteractor._ipm2.bus.on('process:event', function (data) {
        if (data.event !== 'online' || !_this.processes[data.process.name]) return false;

        var rev = data.process.versioning && data.process.versioning.revision ? data.process.versioning.revision : null;

        _this.resetAggregation(data.process.name, {
          rev: rev,
          server: _this.pushInteractor.opts.MACHINE_NAME
        });
      });

      // Process getting offline, delete aggregation
      this.pushInteractor._ipm2.bus.on('process:event', function (data) {
        if (data.event !== 'stop' || !_this.processes[data.process.name]) return false;
        log('Deleting aggregation for %s', data.process.name);
        delete _this.processes[data.process.name];
      });
    ...
    value: function resetAggregation(appName, meta) {
      var _this2 = this;
      ...
      var start = Date.now();
      this.processes[appName].notifier = setInterval(function (_) {
        var elapsed = Date.now() - start;
        // failsafe
        if (elapsed / 1000 > cst.AGGREGATION_DURATION) {
          clearInterval(_this2.processes[appName].notifier);
          _this2.processes[appName].notifier = null;
        }
        ...
        var msg = {
          ...
          process: _this2.processes[appName].process
        };
        _this2.pushInteractor && _this2.pushInteractor.transport.send('axm:transaction:learning', msg);
      }, 5000);

      this.processes[appName].initialization_timeout = setTimeout(function (_) {
        log('Initialization timeout finished for app:%s', appName);
        clearInterval(_this2.processes[appName].notifier);
        _this2.processes[appName].notifier = null;
        _this2.processes[appName].initialization_timeout = null;
    ...
    value: function clearData() {
      ...
      Object.keys(this.processes).forEach(function (process) {
        _this3.resetAggregation(process, _this3.processes[process].process);
      });
    }
    ...
    key: 'initializeRouteMeta',
    value: function initializeRouteMeta(process) {
    ...
    key: 'validateData',
    value: function validateData(packet) {
    ...
    key: 'aggregate',
    value: function aggregate(packet) {
      var _this4 = this;
      ...
      var path = newTrace.spans[0].labels[this.LABELS.HTTP_PATH_LABEL_KEY];
      ...
      newTrace.spans.forEach(function (span) {
        ...
        for (var i = 0, len = _this4.SPANS_DB.length; i < len; i++) {
          if (span.name.indexOf(_this4.SPANS_DB[i]) > -1) {
            process.meta.db_meter.update();
            if (!process.meta.db_histograms[_this4.SPANS_DB[i]]) {
              process.meta.db_histograms[_this4.SPANS_DB[i]] = new Histogram({ measurement: 'mean' });
            }
            process.meta.db_histograms[_this4.SPANS_DB[i]].update(span.mean);
            break;
          }
        }
      });
    ...
    key: 'mergeTrace',
    value: function mergeTrace(aggregated, trace, process) {
      var _this5 = this;
      ...
      var merge = function merge(variance) {
        ...
        if (variance == null) {
          ...
          // parse strackrace
          _this5.parseStacktrace(trace.spans);
          aggregated.variances.push(trace);
        } else {
          // check to see if request is anormally slow, if yes send it as inquisitor
          if (trace.spans[0].mean > variance.histogram.percentiles([0.95])[0.95] && typeof pushInteractor !== 'undefined' && !process.initialization_timeout) {
            ...
            _this5.pushInteractor.transport.send('axm:transaction:outlier', data);
          }
          ...
          _this5.updateSpanDuration(variance.spans, trace.spans, variance.count);
    ...
    key: 'updateSpanDuration',
    value: function updateSpanDuration(spans, newSpans) {
    ...
    key: 'compareList',
    value: function compareList(one, two) {
    ...
    key: 'matchPath',
    value: function matchPath(path, routes) {
    ...
    key: 'launchWorker',
    value: function launchWorker() {
      var _this6 = this;

      this._worker = setInterval(function (_) {
        var normalized = _this6.prepareAggregationforShipping();
        Object.keys(normalized).forEach(function (key) {
          _this6.pushInteractor.transport.send('axm:transaction', normalized[key]);
        });
    ...
    key: 'prepareAggregationforShipping',
    value: function prepareAggregationforShipping() {
      var _this7 = this;

      var normalized = {};

      // Iterate each applications
      Object.keys(this.processes).forEach(function (appName) {
        var process = _this7.processes[appName];
        ...
        };_this7.SPANS_DB.forEach(function (name) {
    ...
    key: 'isIdentifier',
    value: function isIdentifier(id) {
    ...
    key: 'censorSpans',
    value: function censorSpans(spans) {
      var _this8 = this;
      ...
      spans.forEach(function (span) {
        ...
        Object.keys(span.labels).forEach(function (key) {
          if (typeof span.labels[key] === 'string' && key !== 'stacktrace') {
            span.labels[key] = span.labels[key].replace(_this8.REGEX_JSON_CLEANUP, '\": \"?\"'); // eslint-disable-line
          }
        });
      });
    }
    ...
    key: 'parseStacktrace',
    value: function parseStacktrace(spans) {
      var _this9 = this;
      ...
      spans.forEach(function (span) {
        ...
        if (!span || !span.labels || !span.labels.stacktrace || typeof span.labels.stacktrace !== 'string') return;
        ...
        var result = _this9.stackParser.parse(span.labels.stacktrace.stack_frame);
        if (result) {
...
//# sourceMappingURL=TransactionAggregator.js.map