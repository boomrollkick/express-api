...
var log = require('debug')('interactor:client');
var fs = require('fs');
...
var printOut = function printOut(msg) {
  if (process.env.PM2_SILENT || process.env.PM2_PROGRAMMATIC) return false;
  return console.log.apply(console, arguments);
};

module.exports = function () {
  function InteractorDaemonizer() {
    _classCallCheck(this, InteractorDaemonizer);
  }

  _createClass(InteractorDaemonizer, null, [{
    key: 'ping',
     ...
     */
    value: function ping(opts, cb) {
      if (typeof cb !== 'function') {
        throw new Error('Missing parameters');
      } else if ((typeof opts === 'undefined' ? 'undefined' : _typeof(opts)) !== 'object' || !opts || !opts.INTERACTOR_RPC_PORT) {
      ...
      client.sock.once('reconnect attempt', function (_) {
        client.sock.close();
        log('Interactor Daemon not launched');
        return cb(null, false);
      });

      client.sock.once('connect', function (_) {
        client.sock.once('close', function (_) {
          return cb(null, true);
        });
        client.sock.close();
      ...
      client.sock.once('error', function (e) {
        if (e.code === 'EACCES') {
          fs.stat(opts.INTERACTOR_RPC_PORT, function (e, stats) {
            if (stats.uid === 0) {
    ...
    key: 'killInteractorDaemon',
    value: function killInteractorDaemon(conf, cb) {
      var _this = this;

      process.env.PM2_INTERACTOR_PROCESSING = true;

      log('Killing interactor #1 ping');
      this.ping(conf, function (err, online) {
        ...
        _this.launchRPC(conf, function (err, data) {
          if (err) {
            setTimeout(function (_) {
              _this.disconnectRPC(cb);
            }, 100);
            return false;
          }
          _this.rpc.kill(function (err) {
    ...
    key: 'launchRPC',
    value: function launchRPC(conf, cb) {
      var _this2 = this;
      ...
      var generateMethods = function generateMethods(cb) {
        _this2.client.methods(function (err, methods) {
      ...
      this.client.sock.once('reconnect attempt', function (err) {
        _this2.client.sock.removeAllListeners();
    ...
    key: 'daemonize',
    value: function daemonize(cst, conf, cb) {
      ...
      var out = testEnv ? 1 : fs.openSync(constants.INTERACTOR_LOG_FILE_PATH, 'a');
      var err = testEnv ? 2 : fs.openSync(constants.INTERACTOR_LOG_FILE_PATH, 'a');

      var child = childProcess.spawn(process.env.NODEJS_EXECUTABLE || 'node', [InteractorJS], {
        silent: false,
        detached: true,
        cwd: process.cwd(),
        env: Object.assign({
      ...
      try {
        var prevPid = fs.readFileSync(constants.INTERACTOR_PID_PATH);
    ...
    key: 'launchOrAttach',
    value: function launchOrAttach(conf, infos, cb) {
      var _this3 = this;

      this.ping(conf, function (err, online) {
        if (!err && online) {
          log('Interactor online, restarting it...');
          _this3.launchRPC(conf, function (_) {
            _this3.rpc.kill(function (ignoredErr) {
              _this3.daemonize(conf, infos, cb);
            });
          });
        } else {
    ...
    value: function update(conf, cb) {
      ...
      this.ping(conf, function (err, online) {
        ...
        _this4.launchRPC(conf, function (_) {
          _this4.rpc.kill(function (err) {
            if (err) {
              return cb ? cb(err) : printError(err);
            }
            printOut('Interactor successfully killed');
            setTimeout(function (_) {
              _this4.launchAndInteract(conf, {}, function (_) {
    ...
    key: 'getOrSetConf',
    value: function getOrSetConf(cst, infos, cb) {
      infos = infos || {};
      var configuration = {
        version_management: {
          active: true
      ...
      try {
        var fileContent = fs.readFileSync(cst.INTERACTION_CONF).toString();
    ...
    key: 'disconnectRPC',
    value: function disconnectRPC(cb) {
      var _this5 = this;
      ...
      try {
        ...
        this.client_sock.once('close', function (_) {
        ...
        timer = setTimeout(function (_) {
          if (_this5.client_sock.destroy) {
            _this5.client_sock.destroy();
          }
          return cb ? cb(null, { success: true }) : false;
        }, 200);
    ...
    key: 'launchAndInteract',
    value: function launchAndInteract(cst, opts, cb) {
      var _this6 = this;
      ...
      this.getOrSetConf(Object.assign(cst, constants), opts, function (err, conf) {
        ...
        return _this6.launchOrAttach(cst, conf, cb);
      });
    }
  ...
  return InteractorDaemonizer;
}();
//# sourceMappingURL=InteractorClient.js.map